...
controller.right.onEvent(ControllerButtonEvent.Released, function () {
    animation.stopAnimation(animation.AnimationTypes.All, 小王子)
})
...
function drawHPBars () {
    if (playerBar == null || bossBar == null) {
        return
    }
    pImg = image.create(50, 6)
    pImg.fillRect(0, 0, 50, 6, 14)
    let pColor = playerHP > 50 ? 7 : (playerHP > 25 ? 5 : 2)
pImg.fillRect(1, 1, Math.max(0, Math.floor(48 * playerHP / 100)), 4, pColor)
    pImg.drawRect(0, 0, 50, 6, 15)
    playerBar.setImage(pImg)
    bImg = image.create(50, 6)
    bImg.fillRect(0, 0, 50, 6, 14)
    let bColor = isBerserk ? 2 : 4
bImg.fillRect(1, 1, Math.max(0, Math.floor(48 * bossHP / 100)), 4, bColor)
    bImg.drawRect(0, 0, 50, 6, 15)
    bossBar.setImage(bImg)
}
let Boss的火球: Sprite = null
let bImg: Image = null
let pImg: Image = null
let bossBar: Sprite = null
let playerBar: Sprite = null
...
Boss.setPosition(129, 90)
playerBar = sprites.create(image.create(50, 6), SpriteKind.Player)
playerBar.setFlag(SpriteFlag.Ghost, true)
playerBar.z = 100
playerBar.setPosition(32, 6)
bossBar = sprites.create(image.create(50, 6), SpriteKind.Player)
bossBar.setFlag(SpriteFlag.Ghost, true)
bossBar.z = 100
...
scene.setBackgroundImage(img`
    ...
    7777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777
    `)
controller.moveSprite(小王子, 100, 0)
